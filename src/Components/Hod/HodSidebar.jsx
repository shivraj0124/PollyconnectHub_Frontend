...
function HodSidebar({ onNavigate }) {
  ...
  const navItems = [
    { label: "Dashboard", to: "/Hod/Dashboard", icon: RxDashboard      },
    { label: "Projects",  to: "/Hod/Projects",  icon: GoProjectSymlink },
  ];
  ...
  return (
    <>
      <style>{`
        ...
        .hod-sb { font-family: 'Plus Jakarta Sans', sans-serif; }
        .syne   { font-family: 'Syne', 'Plus Jakarta Sans', sans-serif; }
      `}</style>

      <div className={`hod-sb flex flex-col h-full max-md:h-[90vh] w-full justify-between max-h-screen transition-colors duration-300
        ...
        <div>
          ...
          <p className={`px-5 pt-5 pb-2 text-[10px] font-bold uppercase tracking-widest
            ${isDark ? "text-slate-600" : "text-slate-400"}`}>HOD Panel</p>
        ...
        <div className="px-3 pb-5">
          <div className={`rounded-2xl border p-4
            ...
            <span className={`inline-flex items-center text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full mb-3 block w-fit
              ${isDark
