...
function StudentProjects() {
  ...
  const handleSearchC = async (e) => {
    ...
    if (value.length > 0) {
      ...
      setSuggestions(res.data.users); // assuming res.data.users is array of { _id, username }
    } else {
      setSuggestions([]);
    }
  };
                ...
                <div className="p-4">
                  ...
                  <div style={{ width: "300px" }}>
                    <input
                      ...
                      className="focus:outline-none border-b border-gray-300 rounded-md"
                    />
                    ...
                    <ul className="mt-5">
                      {selectedUsers.map((user) => (
                        <li key={user._id} className="px-3 p-1 border rounded-xl w-max bg-green-100 text-green-500 font-semibold">{user.username}</li>
                      ))}
                    </ul>
                  </div>
                  <div className="flex flex-col">
